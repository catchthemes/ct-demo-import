...
<?php
$quantity = get_theme_mod( 'catch_sketch_featured_content_number', 3 );

for ( $i = 1; $i <= $quantity; $i++ ) {
	$target = get_theme_mod( 'catch_sketch_featured_content_target_' . $i ) ? '_blank' : '_self';

	$catch_sketch_catlink = get_theme_mod( 'catch_sketch_featured_content_link_' . $i ) ? get_theme_mod( 'catch_sketch_featured_content_link_' . $i ) : '#';
	...
	<article id="featured-post-' . esc_attr( $i ) . '" class="hentry featured-image-content"><div class="hentry-inner">';
		...
		$catch_sketch_title   = get_theme_mod( 'catch_sketch_featured_content_title_' . $i );
		$content = get_theme_mod( 'catch_sketch_featured_content_content_' . $i );
		$more_button = get_theme_mod( 'catch_sketch_featured_content_more_button_text_' . $i );

		$image = get_theme_mod( 'catch_sketch_featured_content_image_' . $i ) ? get_theme_mod( 'catch_sketch_featured_content_image_' . $i ) : '';
