...
if ( 'post' === $catch_sketch_type || 'jetpack-portfolio' === $catch_sketch_type || 'page' === $catch_sketch_type ) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( 'post' === $catch_sketch_type ) {
			$catch_sketch_post_id = get_theme_mod( 'catch_sketch_portfolio_post_' . $i );
		} elseif ( 'page' === $catch_sketch_type ) {
			$catch_sketch_post_id = get_theme_mod( 'catch_sketch_portfolio_page_' . $i );
		} elseif ( 'jetpack-portfolio' === $catch_sketch_type ) {
...
elseif ( 'category' === $catch_sketch_type ) {
	...
	if ( get_theme_mod( 'catch_sketch_portfolio_select_category' ) ) {
		$args['category__in'] = (array) get_theme_mod( 'catch_sketch_portfolio_select_category' );
	}
...
if ( $loop -> have_posts() ) :
	while ( $loop -> have_posts() ) :
		...
		$show_meta    = get_theme_mod( 'catch_sketch_portfolio_meta_show', 'show-meta' );
