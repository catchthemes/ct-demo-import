 ...
 */
function catch_sketch_custom_controls( $wp_customize ) {
	// Custom control for Important Links.
	class Catch_Sketch_Important_Links extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$important_links = array(
				'theme_instructions' => array(
					'link'  => esc_url( 'https://catchthemes.com/themes/catch-sketch-pro/#theme-instructions' ),
					'text'  => esc_html__( 'Theme Instructions', 'catch-sketch-pro' ),
					),
				'support' => array(
					'link'  => esc_url( 'https://catchthemes.com/support/' ),
					'text'  => esc_html__( 'Support', 'catch-sketch-pro' ),
					),
				'changelog' => array(
					'link'  => esc_url( 'https://catchthemes.com/changelogs/catch-sketch-theme/' ),
					'text'  => esc_html__( 'Changelog', 'catch-sketch-pro' ),
					),
				'facebook' => array(
					'link'  => esc_url( 'https://www.facebook.com/catchthemes/' ),
					'text'  => esc_html__( 'Facebook', 'catch-sketch-pro' ),
					),
				'twitter' => array(
					'link'  => esc_url( 'https://twitter.com/catchthemes/' ),
					'text'  => esc_html__( 'Twitter', 'catch-sketch-pro' ),
					),
				'gplus' => array(
					'link'  => esc_url( 'https://plus.google.com/+Catchthemes/' ),
					'text'  => esc_html__( 'Google+', 'catch-sketch-pro' ),
					),
				'pinterest' => array(
					'link'  => esc_url( 'http://www.pinterest.com/catchthemes/' ),
					'text'  => esc_html__( 'Pinterest', 'catch-sketch-pro' ),
					),
			);
	...
	class Catch_Sketch_Multi_Cat extends WP_Customize_Control {
		...
		public function render_content() {
			$dropdown = wp_dropdown_categories(
				array(
					...
					'hide_if_empty'    => false,
					'show_option_all'  => esc_html__( 'All Categories', 'catch-sketch-pro' ),
				)
			);
			...
			echo '<p class="description">' . esc_html__( 'Hold down the Ctrl (windows) / Command (Mac) button to select multiple options.', 'catch-sketch-pro' ) . '</p>';
		}
	}

	// Custom control for any note, use label as output description.
	class Catch_Sketch_Note_Control extends WP_Customize_Control {
	...
	class Catch_Sketch_Sortable_Custom_Control extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			$sortable_sections = catch_sketch_get_sortable_sections();
		...
		*/
		...
		?>
			<ul class="custom-sortable">
				...
				foreach ( $sortable_sections as $key => $value ) {
					...
					if ( isset( $value['section'] ) ) {
						echo '<a href="javascript:wp.customize.section( \'' . esc_attr( $value['section'] ) . '\' ).focus();">' . esc_html__( 'Edit', 'catch-sketch-pro' ) . '</a>';
					}
					echo '</li>';
				}
			    ?>
			...
			<input id="catch_sketch_sortable_value" type="hidden" <?php $this->link(); ?> value="<?php echo esc_attr( $this->value() ); ?>" />
		<?php
		}
	}

	class Catch_Sketch_Toggle_Control extends WP_Customize_Control {
		public $type = 'light';
...
add_action( 'customize_register', 'catch_sketch_custom_controls', 1 );
