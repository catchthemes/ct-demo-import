...
if ( ! function_exists( 'catch_sketch_posted_on' ) ) :
	...
	function catch_sketch_posted_on() {
		...
		$posted_on = sprintf(
			...
			__( '<span class="date-label"> </span>%s', 'catch-sketch-pro' ),
		...
		$byline = sprintf(
			...
			__( '<span class="author-label screen-reader-text">By </span>%s', 'catch-sketch-pro' ),
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				$output = sprintf( '<span class="cat-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="cat-text screen-reader-text">Categories</span>', 'Used before category names.', 'catch-sketch-pro' ) ),
					$categories_list
				); // WPCS: XSS OK.
			}
		}
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="tags-text screen-reader-text">Tags</span>', 'Used before tag names.', 'catch-sketch-pro' ) ),
					$tags_list
				); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(*/
						...
						/*__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'catch-sketch-pro' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'catch-sketch-pro' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'catch_sketch_entry_posted_on' ) ) :
	...
	function catch_sketch_entry_posted_on() {
		...
		printf( '<span class="posted-on">'  . '<span class="date-label screen-reader-text">%1$s</span><a href="%2$s" rel="bookmark">%3$s</a></span>',
			_x( 'Posted on', 'Used before publish date.', 'catch-sketch-pro' ),
...
if ( ! function_exists( 'catch_sketch_blog_entry_meta_right' ) ) :
	...
	function catch_sketch_blog_entry_meta_right() {
		...
		if ( 'post' === get_post_type() ) {
			...
			printf(
				/* translators: %s: post date */
				__( '<span class="posted-on"><span class="date-label"> </span>%s', 'catch-sketch-pro' ),
...
if ( ! function_exists( 'catch_sketch_header_title' ) ) :
	...
	function catch_sketch_header_title() {
		if ( is_front_page() ) {
			...
			esc_html_e( 'Oops! That page can&rsquo;t be found.', 'catch-sketch-pro' );
		} elseif ( is_search() ) {
			/* translators: %s: search query. */
			printf( esc_html__( 'Search Results for: %s', 'catch-sketch-pro' ), '<span>' . get_search_query() . '</span>' );
...
if ( ! function_exists( 'catch_sketch_header_text' ) ) :
	...
	function catch_sketch_header_text() {
		if ( is_front_page() ) {
			...
			esc_html_e( 'It looks like nothing was found at this location. Maybe try one of the links below or a search?', 'catch-sketch-pro' );
		} elseif( is_search() ) {
	...
	function catch_sketch_single_image() {
		...
		if ( $post) {
	 		if ( is_attachment() ) {
				...
				$metabox_feat_img = get_post_meta( $parent,'catch-sketch-featured-image', true );
			} else {
				$metabox_feat_img = get_post_meta( $page_id,'catch-sketch-featured-image', true );
			}
		}
	...
	function catch_sketch_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php esc_html_e( 'Pingback:', 'catch-sketch-pro' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( 'Edit', 'catch-sketch-pro' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				...
				<div class="comment-container">
					<div class="comment-header">
						...
						<?php printf( __( '%s <span class="says screen-reader-text">says:</span>', 'catch-sketch-pro' ), sprintf( '<cite class="fn author-name">%s</cite>', get_comment_author_link() ) ); ?>
						</header><!-- .comment-meta -->
								...
								<time datetime="<?php comment_time( 'c' ); ?>">
									<?php
										printf(
											...
											esc_html__( '%1$s at %2$s', 'catch-sketch-pro' ),
											get_comment_time( get_option( 'date_format' ) ),
											get_comment_time( get_option( 'time_format' ) )
										);
									?>
								</time></a>
							<?php edit_comment_link( esc_html__( 'Edit', 'catch-sketch-pro' ), '<span class="edit-link">', '</span>' ); ?>

							<?php if ( '0' == $comment->comment_approved ) : ?>
								<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'catch-sketch-pro' ); ?></p>
							<?php endif; ?>
