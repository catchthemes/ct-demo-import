 ...
 */
function catch_sketch_activation_redirect() {
    global $pagenow;

    if ( 'themes.php' === $pagenow && is_admin() && isset( $_GET['activated'] ) ) {
        wp_redirect( esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about' ), 'themes.php' ) ) ) );
    }
}
add_action( 'admin_init', 'catch_sketch_activation_redirect' );

/**
 * Add theme page
 */
function catch_sketch_menu() {
	add_theme_page( esc_html__( 'About Theme', 'catch-sketch-pro' ), esc_html__( 'About Theme', 'catch-sketch-pro' ), 'edit_theme_options', 'catch-sketch-about', 'catch_sketch_about_display' );
}
	...
	<div class="wrap about-wrap full-width-layout">
		...
		<div class="about-theme">
			<div class="theme-description">
				...
				?></p>
				<p class="actions">
					<a href="https://catchthemes.com/themes/catch-sketch-pro" class="button button-secondary" target="_blank"><?php esc_html_e( 'Theme Info', 'catch-sketch-pro' ); ?></a>

					<a href="https://catchthemes.com/themes/catch-sketch-pro/#theme-instructions" class="button button-primary" target="_blank"><?php esc_html_e( 'Theme Instructions', 'catch-sketch-pro' ); ?></a>

					<a href="https://catchthemes.com/demo/catch-sketch" class="button button-secondary" target="_blank"><?php esc_html_e( 'View Demo', 'catch-sketch-pro' ); ?></a>
				</p>
			</div>
		...
		<nav class="nav-tab-wrapper wp-clearfix" aria-label="<?php esc_html_e( 'Secondary menu', 'catch-sketch-pro' ); ?>">
			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['page'] ) && 'catch-sketch-about' === $_GET['page'] && ! isset( $_GET['tab'] ) ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'About', 'catch-sketch-pro' ); ?></a>

			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'free_vs_pro' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['tab'] ) && 'free_vs_pro' === $_GET['tab'] ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Free Vs Pro', 'catch-sketch-pro' ); ?></a>

			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'changelog' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['tab'] ) && 'changelog' === $_GET['tab'] ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Changelog', 'catch-sketch-pro' ); ?></a>

			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'updating_pro' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['tab'] ) && 'updating_pro' === $_GET['tab'] ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Updating Pro Theme', 'catch-sketch-pro' ); ?></a>

			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'import_demo' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['tab'] ) && 'import_demo' === $_GET['tab'] ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'Import Demo', 'catch-sketch-pro' ); ?></a>

			<a href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'license' ), 'themes.php' ) ) ); ?>" class="nav-tab<?php echo ( isset( $_GET['tab'] ) && 'license' === $_GET['tab'] ) ?' nav-tab-active' : ''; ?>"><?php esc_html_e( 'License', 'catch-sketch-pro' ); ?></a>
		</nav>
		...
		<?php
			...
			catch_sketch_updating_pro_screen();

			catch_sketch_import_demo();

			do_action( 'catch_sketch_about_after' );
		?>

		<div class="return-to-dashboard">
			<?php if ( current_user_can( 'update_core' ) && isset( $_GET['updated'] ) ) : ?>
				<a href="<?php echo esc_url( self_admin_url( 'update-core.php' ) ); ?>">
					<?php is_multisite() ? esc_html_e( 'Return to Updates', 'catch-sketch-pro' ) : esc_html_e( 'Return to Dashboard &rarr; Updates', 'catch-sketch-pro' ); ?>
				</a> |
			<?php endif; ?>
			<a href="<?php echo esc_url( self_admin_url() ); ?>"><?php is_blog_admin() ? esc_html_e( 'Go to Dashboard &rarr; Home', 'catch-sketch-pro' ) : esc_html_e( 'Go to Dashboard', 'catch-sketch-pro' ); ?></a>
		</div>
	</div>
	<?php
}
...
function catch_sketch_main_screen() {
	...
	?>
		<div class="feature-section two-col">
			<div class="col card">
				<h2 class="title"><?php esc_html_e( 'Theme Customizer', 'catch-sketch-pro' ); ?></h2>
				<p><?php esc_html_e( 'All Theme Options are available via Customize screen.', 'catch-sketch-pro' ) ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Customize', 'catch-sketch-pro' ); ?></a></p>
			</div>

			<div class="col card">
				<h2 class="title"><?php esc_html_e( 'Got theme support question?', 'catch-sketch-pro' ); ?></h2>
				<p><?php esc_html_e( 'Get genuine support from genuine people. Whether it\'s customization or compatibility, our seasoned developers deliver tailored solutions to your queries.', 'catch-sketch-pro' ) ?></p>
				<p><a href="https://catchthemes.com/support-forum" class="button button-primary"><?php esc_html_e( 'Support Forum', 'catch-sketch-pro' ); ?></a></p>
			</div>
		</div>
	<?php
	}
 ...
 */
function catch_sketch_updating_pro_screen() {
 ...
 */
function catch_sketch_import_demo() {
	...
	?>
		<div class="wrap about-wrap demo-import-wrap">
			...
			<?php if ( class_exists( 'CatchThemesDemoImportPlugin' ) ) { ?>
				<div class="col card">
					<h2 class="title"><?php esc_html_e( 'Import Demo', 'catch-sketch-pro' ); ?></h2>
					<p><?php esc_html_e( 'You can easily import the demo content using the Catch Themes Demo Import Plugin.', 'catch-sketch-pro' ) ?></p>
					<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=catch-themes-demo-import' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Import Demo', 'catch-sketch-pro' ); ?></a></p>
				</div>
				<?php } 
				else {
					...
					<div class="col card">
					<h2 class="title"><?php esc_html_e( 'Install Catch Themes Demo Import Plugin', 'catch-sketch-pro' ); ?></h2>
					<p><?php esc_html_e( 'You can easily import the demo content using the Catch Themes Demo Import Plugin.', 'catch-sketch-pro' ) ?></p>
					<p><a href="<?php echo esc_url( $install_url ); ?>" class="button button-primary"><?php esc_html_e( 'Install Plugin', 'catch-sketch-pro' ); ?></a></p>
				</div>
				<?php } ?>
			</div>
		</div>
