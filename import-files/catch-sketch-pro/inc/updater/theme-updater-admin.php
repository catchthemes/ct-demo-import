...
class EDD_Theme_Updater_Admin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'catch_sketch_about_after', array( $this, 'license_page' ) );
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'catch-sketch-pro' );
			}

			
			$redirect = add_query_arg( array( 'sl_theme_activation' => 'false', 'message' => urlencode( $message ) ), $this->base_url );
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							esc_html__( 'Your license key expired on %s.', 'catch-sketch-pro' ),
					...
					case 'revoked' :
						...
						$message = esc_html__( 'Your license key has been disabled.', 'catch-sketch-pro' );
						break;

					case 'missing' :

						$message = esc_html__( 'Invalid license.', 'catch-sketch-pro' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = esc_html__( 'Your license is not active for this URL.', 'catch-sketch-pro' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'catch-sketch-pro' ), $this->item_name );
						break;

					case 'no_activations_left':

						$message = esc_html__( 'Your license key has reached its activation limit.', 'catch-sketch-pro' );
						break;
		...
		wp_redirect( admin_url( add_query_arg( array( 'page' => 'catch-sketch-about', 'tab' => 'license' ), 'themes.php' ) ) );
		exit();
	...
	function check_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( isset( $license_data->expires ) && 'lifetime' != $license_data->expires ) {
				$expires = wp_date( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) );
 ...
 */
function catch_sketch_admin_notices() {
...
add_action( 'admin_notices', 'catch_sketch_admin_notices' );
